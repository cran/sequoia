...
if (((m<1 .or. m>3) .and. Rel>3) .or. &
  ...
  print *, "getAP: illegal k or m! ", Rel, k, m
  stop
endif
...
if (m==3 .and. k<3) then  ! rel 4,5,6
  ...
  if (m==3) then
    print *, "getAP: m=3 & k=3 not yet implemented"
    stop
...
subroutine makeped(Ng, SpecsIntGlb, SpecsIntMkPed, SpecsDbl, ErrV, GenoFR, &
  ...
  Nd, DumParRF, DumLrRF, DumBYRF,  totLL, AP_OUT)
use Global
implicit none
...
double precision, intent(INOUT) :: LrRF(3*Ng), DumLrRF(3*Ng), TotLL(42), &
  AP_OUT(3*MaxMaxAgePO*5*3)
integer :: ParSib, MaxSibIter, i,k,s, CalcLLR, AgeEffect, nAmbMax, Tmax, IndBYmm(3)
...
call AAtoVd(AP_TMP, 3*MaxMaxAgePO, 5, 3, AP_OUT)
...
integer :: i, j, k, Round, isP(2), PriorPed(nInd, 2), Lboth
...
!  if (MODULO(x,100)==0)  call Rprint("x: ", (/x/), (/0.0D0/), "INT")
!  if (x>=330 .and. x<=350) call Rprint("x: ", (/x, ij/), (/0.0D0/), "INT")
!  if (x>=330 .and. x<=350) call Rprint("x: ", (/x, ij, Parent(ij(1),:), Parent(ij(2),:)/), (/0.0D0/), "INT")
  LL = missing
  ij = PairID(x,:)
  PK = PairType(x)
...
if (complx>0 .and. (focal==2 .or. focal==3) .and. &
 (MaxLL(LL)==LL(3) .or. MaxLL(LL)==LL(2)) .and. Parent(A,3-k)==0) then 
